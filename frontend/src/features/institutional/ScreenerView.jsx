...
export default function ScreenerView() {
  ...
  const load = async () => {
    ...
    try {
      const res = await fetch(`${API}/screener?min_inst_pct=${minPct}&flow=${flow}`)
    ...
    <div style={{ padding: '1.5rem' }}>
      ...
      {data && (
        <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '10px', overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr 1fr 1fr 1fr 1fr', padding: '10px 16px', background: '#1e293b', color: '#64748b', fontSize: '0.72rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
            <span>Ticker</span><span style={{textAlign:'right'}}>Inst %</span><span style={{textAlign:'right'}}>Avg Chg</span><span style={{textAlign:'right'}}>Buyers</span><span style={{textAlign:'right'}}>Sellers</span><span style={{textAlign:'right'}}>Flow</span>
          </div>
          {data.length === 0 && (
            <div style={{ color: '#475569', textAlign: 'center', padding: '3rem' }}>No stocks match these filters</div>
          )}
          {data.map((r, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '80px 1fr 1fr 1fr 1fr 1fr', padding: '10px 16px', borderTop: '1px solid #1e293b', alignItems: 'center' }}>
